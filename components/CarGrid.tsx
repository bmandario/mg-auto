...
export default function CarGrid({ cars }: { cars: Car[] }) {
  ...
  return (
    <div>
      {/* Filter bar */}
      <div className="sticky top-16 z-40 bg-white/95 backdrop-blur-md border-b border-[#e5e7eb] py-3 mb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 text-xs font-bold tracking-widest uppercase border transition-colors ${
                showFilters || activeFilterCount > 0
                  ...
                  : "border-[#d1d5db] text-[#666] hover:border-[#cc1111] hover:text-[#cc1111]"
              }`}
            >
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                className="text-[10px] tracking-widest uppercase text-[#666] hover:text-[#cc1111] transition-colors"
              >
                Clear All
              </button>
            )}
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-[#666] text-xs hidden sm:block">Sort:</span>
              <div className="relative">
                <select
                  value={filters.sort}
                  onChange={(e) => setFilters((f) => ({ ...f, sort: e.target.value }))}
                  className="appearance-none bg-white border border-[#d1d5db] text-[#333] text-xs px-3 py-2 pr-7 focus:outline-none focus:border-[#cc1111] cursor-pointer"
                >
                  {SORT_OPTIONS.map((o) => (
                    <option key={o.value} value={o.value}>{o.label}</option>
                  ))}
                </select>
                <ChevronDown size={12} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#666] pointer-events-none" />
              </div>
            </div>
          </div>
          ...
          {/* Expanded filter row */}
          {showFilters && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3 pt-3 border-t border-[#e5e7eb]">
              {/* Brand */}
              <div className="relative">
                <select
                  value={filters.brand}
                  onChange={(e) => setFilters((f) => ({ ...f, brand: e.target.value }))}
                  className="w-full appearance-none bg-white border border-[#d1d5db] text-[#333] text-xs px-3 py-2.5 pr-7 focus:outline-none focus:border-[#cc1111]"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        <p className="text-[#666] text-sm">
          <span className="text-[#0a0a0a] font-semibold">{filtered.length}</span> car{filtered.length !== 1 ? "s" : ""} found
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-[#666] text-lg mb-2">No cars match your filters.</p>
